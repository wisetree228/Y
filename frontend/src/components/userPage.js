...
const UserChannel = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setFriend(userResponse.data.friendship); // Исправлено на правильное место
    ...
    const handleFriendship = async () => {
        try {
            // Здесь должна быть реальная URL-адреса для добавления/удаления в друзья
            const friendshipResponse = await axios.post(`${API_BASE_URL}/friendship`, { userId: user_id });
            setFriend(friendshipResponse.data); // Исправлено
            console.log(isFriend ? 'Подписан' : 'Отписан'); // Исправлено
        } catch (error) {
